...
## 附录补 2（R3 轮，2026-09-03 02:10）

- **B9 已修**（机械类）：00 对齐表悬空引用改为「内部台账不随包，以飞书 wiki 原文为准」；
  04 删「前 30」字样（该段本就「不作确定表述」）。v3 重打全绿。
- **B5 实勘结论**：「21 锚 14 进块」在盘上任何位置（真相源 demo/gap-probe、初赛提交包、
  公开仓 tag）均无 .ots 回执实物。两个选项待作者拍板：①作者手上有回执 → 补文件入包；
  ②没有 → README §五改为弱表述（「时间锚提交记录在作者本地，可应评委要求出示」）。
- **当前候选哈希**：with-panel `ff5d13fc…c3e1` / no-panel `8d81c29b…0f58d`（B9 修复后），
  check-v3 双变体红点 0/0。

## 附录补 3（R4 轮，2026-09-03 02:50）

- **pi PPT 审计**（15 张逐页抽文本 vs 06 大纲）：两个硬缺口——大纲页 3.5「量程阶梯+自主探索」
  整页缺失（PPT 无 130/proposer/oracle/policy 任何字样）；slide13 仓库三件套是占位文字
  （「链接核实中」）。其余 5 个关键数字（11364/1.9384/p=0.146/p=0.6875）逐字一致。
- **已产修订版副本**：`方案说明PPT-格子AI工作室-陈东.修订版-0903.pptx`（原版 WPS 锁定未动）。
  机械事实类 4 修复：填仓库三件套 / 补右括号 / 新增量程阶梯页并挪到第 10 位 / 补双包解压说明。
  python-pptx 9 项内容验证全 PASS。**待作者过目后替换原版再入包。**
- sol R4 纪律入账：v3 不再无意义复跑；凌晨不再买新评审；待拍板六项不替作者决定。

## 附录补 4（R4b 轮，2026-09-03 03:25）

- **sol 把关**：修订版 4 处机械修改逐项验真 5/5 PASS，总判「可作用户过目基础」，
  附加要求=渲染检查第 10/12 页溢出。
- **pi 校对**：8 项旧口径词全部清零（链接核实中/200条/Node18/前30/正在补做/self-attesting/
  取象主标题/裸引号）；抓到 5 个小瑕疵，已二轮微修：双右括号→单、孤立引号删除、
  修辞逗号→破折号、仓库链接括号配平、页码 15→16 全局改+新页补页脚+解包框上移。
- 修订版现状：16 页，内容判据全绿，**待用户 WPS 过目（重点第 10/12 页渲染）后入包**。

## 附录补 5（R4c 轮，2026-09-03 03:40）

- **渲染检查完成**（sol 附加条件的收口）：WPS COM 导出 16 页 PNG，视觉模型逐项检查——
  第 10 页（新量程页）无溢出/无重叠/层级清晰；第 12 页（解包说明框）不压正文不压页脚。
  顺手采纳视觉建议：三档数据行（random 50%/平凡解 77.6%/oracle 100%）放大加粗变蓝，重渲染终验通过。
- **修订版 PPT 全部技术关卡清零**：内容判据（sol 5/5）+ 旧口径（pi 8/8）+ 渲染（视觉 3/3）。
  现在只差用户过目拍板。

## 终局记录（2026-09-03 12:30）

**用户拍板：本届复赛不参加了**（作品与其他队伍重复）。本分支与全部冲刺产物转为**存档状态**：
v3 候选包、20 页 PPT 终版、判据链、会审记录全部保留在盘（D:/tmp/quxiang-longtask/），
不再执行提交落位。队伍转向 2origin 本象 AI 计算机题目（决赛 9/22 杭州）。

冲刺本身不白费：冷启动评审、判据链、多AI会审的方法论已沉淀为 task-router/longtask-mode
技能的可复用资产，「作品撞题」本身的教训也入账——发现撞题后 14 小时内快速止损、把火力
集中到差异化赛道，是比硬交一个「重复的作品」更重要的决策。
